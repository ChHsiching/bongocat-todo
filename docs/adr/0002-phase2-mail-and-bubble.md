...
**决策**：桌宠右键菜单新增「邮件列表」项（在待办/快速新建下面）。打开后展示所有被气泡提醒过的邮件（本地缓存发件人+主题，不含正文）。点击→跳 webmail（不在 app 内看正文，守住 D5-actual）。

**留存规则**：
- 新邮件到达触发气泡后，自动进入邮件列表（未读状态）。
- **默认留存 24 小时**后自动归档（转入归档状态，从主列表移除）。
- **点击后**标记为本地已读 + **5 分钟后**自动归档。
- 归档列表在邮件设置页（D6 的 preference 邮件页）可查看，归档项可再次点击跳转 webmail。
...
**侵入点新增**：
- `src/composables/useAppMenu.ts`：右键菜单加「邮件列表」项（追加，和 todo 菜单项并列）。
- `src/constants/index.ts`：`WINDOW_LABEL.MAIL_LIST` 或复用伴随窗口。
- `src/router/index.ts`：`/mail-list` 路由（追加）。
- `src-tauri/tauri.conf.json`：邮件列表窗口配置（照抄 todo 面板的伴随窗口配置）。
