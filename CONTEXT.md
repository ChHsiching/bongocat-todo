...
| **邮件列表（mail list）** | 桌宠右键菜单入口，本地缓存被气泡提醒过的邮件（发件人+主题）。纯本地状态（已读/归档），不碰邮箱服务端。留存规则：默认 24 小时后归档；点击标本地已读 + 5 分钟后归档 |
| **归档（archived）** | 邮件列表中超过留存时限的邮件转入归档状态，在设置页可查看归档列表并再次点击跳转 webmail |
...
- **邮件列表（D9）**：桌宠右键菜单项（在待办/快速新建下面），本地缓存被气泡提醒过的邮件。留存规则：默认 24 小时后归档；点击标本地已读 + 5 分钟后归档。设置页可查看归档列表并再次跳转。
